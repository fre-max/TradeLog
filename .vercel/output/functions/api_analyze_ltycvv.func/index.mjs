...
//#endregion
//#region .vercel/output/assets/_gemini-DZ_ByW0X.js
...
var GEMINI_MODEL_DEFAUT = "gemini-3.1-flash-lite";
...
function getGeminiVisionModel() {
	const modele = process.env.GEMINI_MODEL?.trim() || "gemini-3.1-flash-lite";
	if (MODELES_GEMINI_OBSOLETES.has(modele)) {
		console.warn(`⚠️ [Gemini] Modèle "${modele}" obsolète — utilisation de ${GEMINI_MODEL_DEFAUT}`);
...
function getEffectiveRetryDelayMs(error) {
	const parsed = parseGeminiRetryDelayMs(error);
	if (process.env.VERCEL) return Math.min(parsed, 4e3);
	return parsed;
}
var MAX_GEMINI_RETRIES = process.env.VERCEL ? 2 : 3;
...
async function optimiserImagePourGemini(input) {
	...
	const optimized = await (0, import_lib.default)(input).rotate().resize({
